...
- 2026-09-25 — 마지막 필드 로그로 가스 판정 수정 효과 확인 후 마무리.
  1. **가스 연기가 서서히 옅어지는 것에 맞춰 효과도 서서히 약해짐** — 전에는
     투척 후 30초가 되는 순간 가스 효과가 뚝 끊겼는데(연기는 아직 남아
     있는데도), 이제 30초 이후 15초에 걸쳐 가스의 최대 강도가 점점 낮아져
     45초에 사라집니다. 그 사이 봇의 능력치 저하도 같이 줄어들고, 노출
     강도가 35% 아래로 내려가면 눈먼 상태도 풀립니다. 15초는 시작값이라
     연기가 사라지는 체감에 맞춰 조정 가능합니다.
  2. **가스 확인용 로그 삭제** — `[GasExposure]`, `[FlashedLayer]` 로그는
     원인 확인이 끝나서 뺐습니다. SAIN DLL 빌드 시각 로그와 문 관련 로그는
     그대로 둡니다.
  - 게임 내 재테스트 필요(문법 검사만 통과).
- 2026-09-25 — 세 번째 필드 로그(새 빌드 확인됨)로 "연기 속에서 가스
  상태가 풀림" 원인 확정 및 수정.
  - 원인: 봇이 가스 판정 반경 4m 경계에 서 있으면서 몇 초마다 들어갔다
    나갔다를 반복했습니다(로그에 "4.0m에서 나감 → 4.0m에서 들어옴"이
    반복). 나갈 때마다 3초 뒤 눈먼 상태가 풀리고 효과가 처음부터 다시
    쌓여서, 교전 중 가스 상태가 뚝 끊기는 것처럼 보였습니다. 다른 행동이
    끼어들어 풀린 경우는 0건이었습니다.
  - 수정: 들어갈 때는 4m, 나갈 때는 6m 기준으로 판정합니다. 한번 가스
    안으로 판정되면 6m 밖으로 확실히 나가야 풀립니다(가스 모드의 실제
    구름 최대 크기 8m 안쪽).
  - 참고: 투척 후 30초가 지난 가스탄 옆에 서 있는 경우도 여러 번 기록됨.
    그때는 원래 설계대로 효과가 없습니다.
- 2026-09-25 — 두 번째 필드 로그 분석 반영.
  1. **오래된 가스탄이 새 수류탄 회피를 가로채던 문제** — 봇은 "가장 가까운
     수류탄 하나"를 골라 반응하는데, 절대 반응할 일이 없는 수류탄(일반
     연막탄, 회피 시간 5초가 지난 CS 가스탄)도 후보에 들어가 있었습니다.
     그래서 바로 옆에 오래된 가스탄이 있으면 조금 더 멀리 새로 날아온
     수류탄을 무시해 버렸고, 비슷한 거리에 새 가스탄/옛 가스탄이 있으면
     회피 ↔ 무반응이 10초 안에 5번씩 바뀌기도 했습니다(로그로 확인). 이제
     반응할 일이 없는 수류탄은 후보에서 뺍니다. 가스 노출 효과는 별도로
     전체 목록을 보기 때문에 영향이 없습니다.
  2. **로그에 SAIN DLL 빌드 시각 표시** — 게임 시작 시 `[SAIN] Loaded DLL
     ... built/modified at 날짜 시간` 한 줄을 남깁니다. 어떤 빌드로 찍힌
     로그인지 바로 확인하기 위함입니다(이번에 로그만으로 빌드를 구분하지
     못해 혼동이 있었음).
  3. (이후 삭제됨) 확인용 로그 추가: 투척 후 30초 노출 시간이 지난 가스탄 반경 안에 봇이
     서 있으면 `[GasExposure] ... past the 30s exposure window` 한 줄(가스
     효과는 없지만 연기는 남아 있을 수 있는 상황).
  - 게임 내 재테스트 필요(문법 검사만 통과).
- 2026-09-24 — 필드 테스트 피드백 반영 (문간 확인 + 확인용 로그).
  1. **앞 봇이 뒤따라오던 봇 앞에서 문을 닫던 문제** — 문이 많은 구역에서
     먼저 들어간 봇이 다시 나가면서 문을 닫았고, 뒤따르던 봇이 문턱에
     걸친 채 문을 뚫고 지나간 것처럼 보였습니다. 문을 닫기 전에 누가 문간
     (1.5m 안)에 서 있는지 확인하지 않았기 때문입니다. 이제 문간에 다른
     봇이나 플레이어가 있으면 닫지 않고 기다렸다가, 비면 닫습니다.
  2. **"연기 속에서 쏘면 가스(flashed) 상태가 풀리는 것 같다" 확인용 로그**
     추가. 원인 후보가 3가지(봇이 판정 반경 4m를 벗어남 / 투척 후 30초
     노출 시간이 끝났는데 연기만 남아 있음 / 다른 행동 레이어가 끼어듦)라
     로그로 가려냅니다. 로그 머리말: `[GasExposure]`(가스 들어감/나감과
     이유/완전 회복), `[FlashedLayer]`(아직 flashed인데 다른 레이어로
     넘어감 — 넘어간 레이어 이름 표시).
  3. 문 관련 확인용 로그: `[DoorOpener] ... held off closing door`(문간에
     사람이 있어서 닫기 보류), `... opening closest in-range shut door as
     fallback`(문에 끼어서 가장 가까운 문을 대신 연 경우).
  - 게임 내 재테스트 필요(문법 검사만 통과).
- 2026-09-24 — 문 수정 방향 재점검 + CS 가스 효과 개선 (3건).
  1. **봇이 지나가던 옆문을 괜히 여닫던 문제** — 문에 끼었을 때 쓰려고 넣은
     "앞에 문이 안 잡히면 3m 안의 가장 가까운 문을 대신 고르기" 기능
     (`DoorOpener`, 봇의 문 열기/닫기 담당)이 끼지 않았을 때도 매 이동마다
     작동해서, 복도 옆 닫힌 문을 열거나 다른 봇·플레이어가 열어 둔 문을
     지나가면서 닫아 버릴 수 있었습니다(분대가 줄지어 지나갈 때 뒤따르는
     봇이 앞 봇이 연 문을 닫는 식). 이제 **실제로 문에 막혀 멈춘 게 확인된
     직후 1초 동안만**, 그리고 **닫힌 문을 여는 것만** 합니다.
  2. **문을 지나간 뒤에도 느리게 걷던 문제** — 문 3m 안에서 걸던 25% 감속을
     벗어날 때 풀어주는 코드가 없어서, 엄폐물로 이동하거나 수류탄을 피하는
     중인 봇은 행동이 끝날 때까지 느릿느릿 움직일 수 있었습니다. 이제 문
     근처를 벗어나는 순간 원래 속도로 돌아갑니다(`SAINMoverClass`, 봇 이동
     담당).
  3. **CS 가스 효과가 서서히 강해지고 서서히 회복되도록 변경** — 기존에는
     섬광탄 효과를 그대로 가져다 써서 켜짐/꺼짐만 있었습니다. 이제 가스
     안에 있으면 약 4초에 걸쳐 명중률/시야/청각 저하가 최대까지 올라가고,
     가스 밖으로 나오면 약 8초에 걸쳐 회복합니다. "눈먼 상태" 행동(더듬어
     찾기, 눈먼 사격)은 노출이 35% 넘게 쌓였을 때(약 1.4초)부터 시작되고,
     가스를 벗어난 뒤 3초 동안 이어집니다. 진짜 섬광탄을 맞으면 가스
     효과에 더해서 겹쳐 적용됩니다. 수치(4초/8초/35%)는 시작값이라 체감에
     따라 조정이 필요할 수 있습니다.
  - 전부 게임 내 재테스트 필요(이 환경에서는 문법 검사만 통과).
