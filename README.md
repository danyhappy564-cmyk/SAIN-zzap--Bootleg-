...
- 2026-09-24 — 코드 재점검으로 CS 가스탄 노출 효과의 버그 3개를 찾아 수정.
  가스 안에 있는 동안 2초마다 섬광탄 효과(`BotFlashedClass.ApplyFlash`,
  봇이 섬광탄에 맞았을 때 쓰는 함수)를 처음부터 다시 걸고 있었는데, 이
  함수는 "방금 섬광에 맞음" 순간에만 해야 할 일까지 매번 다시 해서 생긴
  문제입니다:
  1. **가스 안에서 봇이 절대 반격하지 않음** — 눈먼 사격(blind fire)은
     섬광 후 4초가 지나야 가능한데, 2초마다 이 대기 시간이 리셋돼서
     영원히 사격 불가였습니다.
  2. **눈이 먼 봇이 플레이어 위치를 실시간 추적(투시)** — "마지막으로 본
     적 위치"가 2초마다 플레이어의 현재 실제 위치로 갱신됐습니다.
  3. **분대 수색 지점이 2초마다 중복 추가** — 같은 분대 봇들이 계속 가스
     지점을 수색하러 몰려들 수 있었습니다.
  - 추가로, 이미 더 긴 진짜 섬광탄을 맞은 봇에게 가스의 짧은 효과가
    덮어써져 섬광이 일찍 풀리는 문제도 막았습니다.
  - 수정: 처음 노출될 때만 기존 방식으로 효과를 걸고, 이후에는 지속시간만
    연장하는 새 함수(`RefreshFlash`)를 사용합니다. 게임 내 재테스트 필요.
- 2026-09-21 — CS 가스탄 노출 반경 5m → 4m로 축소(필드 피드백).
- 2026-09-21 — 가스 노출 중인 봇이 기침/신음 소리를 내도록 추가. CS
  가스탄 모드는 플레이어한테만 그 소리를 재생해서 봇은 원래 무음이었음
  — 같은 트리거 조합(모드가 플레이어 기침에 쓰는 것과 동일)을 SAIN
  자체 보이스 시스템으로 재현, 노출 중 3.5초 간격으로 재생.
- 2026-09-21 — CS 가스탄 회피 시간 2초 → 5초로 재조정(같은 날 필드
  테스트로 4초→2초 단축 직후, 2초는 너무 짧아서 몸이 움직이기도 전에
  끝나버린다는 피드백 반영).
- 2026-09-21 — CS 가스탄 회피 시간 4초 → 2초로 단축(필드 피드백: 4초는
  너무 길고 대부분 도망은 그 전에 끝난다는 의견).
- 2026-09-21 — 추가 필드 리포트: "던지자마자 바로 플래시돼서 피할 틈이
  없음". 원인은 섬광탄 레이어(우선순위 85)가 회피 레이어(80)보다 높아서,
  노출 판정이 걸리는 즉시 회피 동작이 다 돌기도 전에 끊겼던 것 — 이제
  회피(4초)가 다 끝난 뒤에만 노출 효과가 시작되도록 수정. 지속시간/
  갱신주기도 늘려서(1.5초/1초 → 3초/2초) 매번 최고강도로 리셋되지
  않고 기존 회복 단계가 실제로 재생되도록 완화(완전한 페이드는 아니지만
  덜 딱딱함). 반경도 8m → 5m로 축소(체감보다 넓다는 피드백 반영).
- 2026-09-21 — 실전 테스트로 CS 가스탄 관련 버그 2개 발견/수정.
  ① "봇이 안 피함": 엎드리기 판정이 프래그 전용 3.5초 퓨즈 상수로
  계산되는데 CS 가스탄은 실제 퓨즈가 없어서 이 값이 몇 초 안에 0이
  됨 — 그러면 8m 이내에서 "터지기 전에 못 피함"으로 오판정돼서 눈에
  띄게 도망치는 대신 그냥 엎드려버렸음(회피 안 하는 것처럼 보인 원인).
  CS 가스탄이면 이 엎드리기 판정 자체를 건너뛰도록 수정.
  ② "능력치 저하 레이어가 깜빡임": 수류탄이 여러 개 동시에 날아다니는
  상황(로그에서 최대 24개 동시 추적 확인)에서, 가스 노출 판정이
  "지금 제일 가까운 위협 하나"만 보는 구조라서 더 가까운 다른 프래그가
  생길 때마다 갱신이 끊겨 효과가 만료됐다 재적용되길 반복했음 —
  추적 중인 수류탄 전체를 훑도록 바꿔서 다른 위협이 끼어들어도 안
  끊기게 수정.
- 2026-09-21 — 블랙 디비전은 CS 가스탄에 반응/능력치 저하 둘 다 이미
  무시하고 있었지만, 추적 목록엔 여전히 등록돼서 매 틱 불필요하게
  갱신되던 낭비 제거. 이제 블랙 디비전이면 CS 가스탄을 애초에 추적
  목록에 넣지도 않음 — 다른 수류탄 종류는 그대로 정상 추적.
- 2026-09-21 — 레이드 도중에 스폰된 봇(웨이브 스폰, 증원 호출, APBS류
  지속 스폰 모드)이 이미 활성화된 CS 가스탄을 영원히 인식 못 하던
  사각지대 수정. 수류탄 추적이 "던져지는 순간 딱 한 번" 쏘는 이벤트
  기반이라, 그 순간에 아직 존재하지 않던 봇은 나중에 가스 구역으로
  걸어 들어와도 회피/능력치 저하가 전혀 안 걸렸음 — 프래그는 3.5초면
  사라져서 거의 안 드러나던 문제인데 가스는 최대 30초라 실제로 걸릴
  가능성이 있었음. 봇이 스폰될 때(컴포넌트 초기화 시점) 맵에 현재
  살아있는 수류탄 목록을 한 번 스캔해서 놓친 CS 가스탄을 추가로
  등록하도록 수정 — 스폰 시 1회성 작업이라 매 틱 비용은 없음.
